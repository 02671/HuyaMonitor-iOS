...
- Instructions:
  ...
  - AVPlayer will not invoke `AVAssetResourceLoaderDelegate` for `https://` URLs. `AVURLAssetHTTPHeaderFieldsKey` only applies to the first playlist request; subsequent TS/fMP4 segments go out as `AppleCoreMedia/...` without Referer and Huya's CDN answers 403, which looks like an audio reconnect loop. Playback must use a custom scheme (`hyhls://`) plus `StreamLoader` so every playlist and segment is fetched with the browser `User-Agent` / `Referer` / `Origin`.
