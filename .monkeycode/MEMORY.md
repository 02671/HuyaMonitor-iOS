...
- Instructions:
  ...
  - AVPlayer will not invoke `AVAssetResourceLoaderDelegate` for `https://` URLs. `AVURLAssetHTTPHeaderFieldsKey` only applies to the first playlist request; subsequent TS/fMP4 segments go out as `AppleCoreMedia/...` without Referer and Huya's CDN answers 403. A custom scheme plus resource-loader cannot feed HLS segments either: CoreMedia fails with error -12881 ("custom url not redirect"). Playback must go through a local loopback HTTP reverse proxy (`StreamProxy` in `StreamLoader.swift`) so AVPlayer uses `http://127.0.0.1` while every playlist and segment is fetched with the browser `User-Agent` / `Referer` / `Origin`.
  - iOS audio follows Windows `audio.py`: one loop, `REFRESH_SEC=120`, `OVERLAP_SEC=2.0`, `ratio=2000`. Keep the current AVPlayer playing while a new one is started; after 2s kill the leftover. On spawn/play death, bump `lineIndex` and retry. Do not bump StreamProxy generation during overlap refresh (that 410s the outgoing player's segments). HLS playlist rewrite must inherit the upstream playlist query (`wsSecret`/`wsTime`/`ratio`); `URL(relativeTo:)` drops it. Proxy `/p?u=` must percent-encode `&` `=` `?` or the signature is truncated.
