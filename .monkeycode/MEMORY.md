...
- Instructions:
  ...
  - Audio refresh/overlap: while still playing, resign every 90 seconds with a 2.0s overlap. On HTTP 403 / CoreMedia -12660 / "Cannot Open" / permission errors, treat the URL as expired: hard-cut and resign on the SAME CDN line first; after 3 same-line failures, bump bitrate (500→next advertised, often 2000) then switch lines. HLS playlist rewrite must inherit the upstream playlist query (`wsSecret`/`wsTime`/`ratio`); `URL(relativeTo:)` drops it. Proxy `/p?u=` must percent-encode `&` `=` `?` or the signature is truncated.
